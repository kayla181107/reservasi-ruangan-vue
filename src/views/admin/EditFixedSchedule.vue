<template>
  <div class="min-h-screen bg-white p-6">
    <div class="max-w-4xl mx-auto">
        ...
        <h1 class="text-3xl font-bold text-gray-900">Edit Jadwal Tetap</h1>
      </div>
...
onMounted(async () => {
  ...
  try {
    ...
    const [roomRes, scheduleRes] = await Promise.all([
      ...
      axios.get(`http://127.0.0.1:8000/api/fixed-schedules/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      }),
    ])

    roomsList.value = roomRes.data.data || roomRes.data
    const data = scheduleRes.data.data || scheduleRes.data
...
async function update() {
  ...
  try {
    ...
    const payload = {
      ...form.value,
      start_time: formatTimeToHHMM(form.value.start_time),
      end_time: formatTimeToHHMM(form.value.end_time),
    }

    await axios.put(`http://127.0.0.1:8000/api/admin/fixed-schedules/${id}`, payload, {
      headers: { Authorization: `Bearer ${token}` },
    })
    ...
    console.error(e)
    Swal.close()
    Swal.fire('Gagal', e.response?.data?.message || 'Gagal menyimpan perubahan.', 'error')
  }
}
</script>
